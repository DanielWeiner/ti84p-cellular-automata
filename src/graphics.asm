...
DrawRect: ; D = x, E = y, H = length, L = height, B = 0: white, 1: black, 2: xor, default: black, Destroys all registers
    ...
    CP 0
    JR Z, __SetOperationAnd
    CP 1
    JR Z, __SetOperationOr
    CP 2
    JR Z, __SetOperationXor
__SetOperationOr:
    LD A, Opcode.OrHL
    LD (__Operation), A
    JR __FindScreenAddress
__SetOperationAnd:
    LD A, Opcode.AndHL
    LD (__Operation), A
    JR __FindScreenAddress
...
__FindScreenAddress:
    ...
    LD D, B ; D = color
    LD E, A ; Bit offset in E
    LD B, A ; Bit offset in B
...
__WriteMultiByte:
    PUSH BC
    LD A, D ; Load the high mask into A for further processing
__MultiByteHighOperation:
    OR (HL)
    LD (HL), A
    INC HL
__MiddleByteLoop:
__MultiByteMiddleValue = $+1
    LD A, 0
__MultiByteMiddleOperation:
    OR (HL)
    LD (HL), A
    INC HL
    DJNZ __MiddleByteLoop
    LD A, E
__MultiByteLowOperation:
    OR (HL)
    LD (HL), A
...
__OneByte:
    LD A, C
    LD (__OneByteOperation), A
    CP Opcode.AndHL
    JR NZ, __OneByteNotWhite
    LD A, Opcode.OrE
